...
async function deleteFromGitHub(slug: string) {
  const filePath = `content/kb/${slug}.md`;
  const apiBase = `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${encodeURIComponent(filePath)}`;
  ...
  if (!getRes.ok) {
    ...
    throw new Error(`读取 GitHub 文件失败：${text}`);
  }
  ...
  if (!deleteRes.ok) {
    ...
    throw new Error(`删除 GitHub 文件失败：${text}`);
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({
      ok: true,
      mode: GITHUB_TOKEN ? 'github' : 'local',
    });
  } catch (error) {
    return NextResponse.json(
